...
from model_selection import (
    get_sclc_model, get_pipeline, TorchVisionResNet2DClassifier, MILResNet50Classifier,
)
...
from training.train_mil import simple_collate_fn_mil, train_epoch_mil, validate_epoch_mil
from data.data_loader import create_dataset
from data.dataset_2d import create_dataset_2d
from data.dataset_mil import create_dataset_mil_bag, create_dataset_whole_slice
...
def parse_args():
    ...
    parser.add_argument("--model-type", type=str, default="swin_unetr",
                        ...
                        help="Model architecture to use. '_2d' uses the per-slice 2D pipeline; "
                             ...
                             "(ResNet50 / DenseNet121); 'mil_resnet50' uses the MIL pipeline "
                             "(whole-slice per-slice DAPT + attention-MIL bag finetune); "
                             "'swin_unetr' uses the full 3D pipeline.")
    ...
    # MIL pipeline knobs
    parser.add_argument("--img-size-mil", type=int, default=384,
...
def main():
    ...
    if pipeline == "mil" and args.mode in ("full", "dapt"):
        # mil_resnet50: DAPT phase reuses TorchVisionResNet2DClassifier; the
        # MIL classifier is rebuilt at fine-tune time and inherits the DAPT
        # backbone weights.
    ...
    if args.mode in ["full", "finetune"]:
        ...
        if pipeline == "mil" and not isinstance(model, MILResNet50Classifier):
    ...
    if args.mode in ["full", "inference"]:
        ...
        if args.mode == "inference":
            ...
            if 'best_finetune_ckpt' in locals() and best_finetune_ckpt:
                ...
                if not isinstance(model, MILResNet50Classifier):
                    model.load_state_dict(torch.load(best_dapt_ckpt, map_location=device))
