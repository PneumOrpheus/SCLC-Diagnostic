...
#         end of --mode finetune by sclc.main design.)
...
if "$PY" -m sclc.main --mode finetune --config "$CONFIG" --model-checkpoint "$DAPT_PBEST" \
...
if "$PY" -m sclc.main --mode inference --config "$CONFIG" --model-checkpoint "$FT_PBEST" \
